...
//youresam's hprm stuff
// The "Hprm" userdata object.
// ------------------------------
UserdataStubs(Hprm, u32)


static int Hprm_read(lua_State *L)
{
	int argc = lua_gettop(L);
	if(argc) return luaL_error(L, "Argument error: Hprm.read() cannot be called from an instance.");
	u32* rmt = pushHprm(L);
	sceHprmPeekCurrentKey(rmt);
	return 1;
}

#define CHECK_CTRL_HPRM(NAME, BIT) \
static int NAME(lua_State *L) \
{ \
    if (lua_gettop(L) != 1) return luaL_error(L, g_errorMessage); \
    u32* a = toHprm(L, 1);\
	lua_pushboolean(L, (*a & BIT )); \
	return 1; \
}
CHECK_CTRL_HPRM(Hprm_playpause,PSP_HPRM_PLAYPAUSE)
CHECK_CTRL_HPRM(Hprm_forward,PSP_HPRM_FORWARD)
CHECK_CTRL_HPRM(Hprm_back,PSP_HPRM_BACK)
CHECK_CTRL_HPRM(Hprm_volup,PSP_HPRM_VOL_UP)
CHECK_CTRL_HPRM(Hprm_voldown,PSP_HPRM_VOL_DOWN)
CHECK_CTRL_HPRM(Hprm_hold,PSP_HPRM_HOLD)

static int Hprm_headphone(lua_State *L)
{
	lua_pushboolean(L, sceHprmIsHeadphoneExist() == 1);
	return 1;
}
static int Hprm_remote(lua_State *L)
{
	lua_pushboolean(L, sceHprmIsRemoteExist() == 1);
	return 1;
}
static int Hprm_microphone(lua_State *L)
{
	lua_pushboolean(L, sceHprmIsMicrophoneExist() == 1);
	return 1;
}

static int Hprm_equal(lua_State *L) {
	u32* a = toHprm(L, 1);
	u32* b = toHprm(L, 2);
...
static const luaL_reg Hprm_methods[] = {
	{"read",     Hprm_read},
	{"playpause", Hprm_playpause},
	{"forward", Hprm_forward},
	{"back", Hprm_back},
	{"volup", Hprm_volup},
	{"voldown", Hprm_voldown},
	{"hold", Hprm_hold},
	{"headphone", Hprm_headphone},
	{"remote", Hprm_remote},
	{"microphone", Hprm_microphone},
  {0, 0}
};
static const luaL_reg Hprm_meta[] = {
  {"__eq", Hprm_equal},
  {0, 0}
};
UserdataRegister(Hprm, Hprm_methods,Hprm_meta)
