...
static int Mp3_load(lua_State *L)
{
	...
	FILE* soundFile = fopen(mp3path, "rb");
	if (!soundFile) return luaL_error(L, "can't open sound file %s.", mp3path);
	fclose(soundFile);
	lua_gc(L, LUA_GCCOLLECT, 0);
...
static int Mp3_play(lua_State *L)
{
	int argc = lua_gettop(L);
	if(argc != 0 ) return luaL_error(L, "wrong number of arguments");
	if (mp3playing == false && mp3init == true){
	irsmp3Play(mp3path, MP3_REPEAT, 0);
	}
	mp3playing = true;
	return 0;
}
...
static int Mp3_volume(lua_State *L)
{
	...
	irsmp3Volume(luaL_checkint(L, 1));
	return 0;
}
