...
static int Wlan_useConnectionConfig(lua_State* L){
  if (!wlanInitialized) return luaL_error(L, wlanNotInitialized);
  int argc = lua_gettop(L);
  if (argc != 1 && argc != 2) return luaL_error(L, "Argument error: index to connection config expected and an optional boolean for auto/manual state checking.");

  int connectionConfig = luaL_checkint(L, 1) - 1;
  int result = sceNetApctlConnect(connectionConfig);

  if (argc == 1 || (argc == 2 && lua_toboolean(L,2) == false)){
    int state = 0;
...
static int Wlan_getState(lua_State* L){
  if (lua_gettop(L) != 0) return luaL_error(L, "no arguments expected.");
  int ret = 1;
  int state = 0;
...
static int Wlan_GetSwitchState(lua_State *L){
  if (lua_gettop(L) != 0) return luaL_error(L, "no arguments expected.");
...
static int Wlan_GetEtherAddr(lua_State *L){
  if (lua_gettop(L) != 0) return luaL_error(L, "no arguments expected.");
  // char themac[18];
  ...
  // unsigned char *macAdd = malloc(sizeof(char)*20);
  // sceWlanGetEtherAddr(macAdd);
  // lua_pushstring(L,macAdd);
  // free(macAdd);
  return 0;
}
...
static int Socket_connect(lua_State *L){
  if (!wlanInitialized) return luaL_error(L, wlanNotInitialized);
  int argc = lua_gettop(L);
  if (argc != 2 && argc !=3) return luaL_error(L, "host, port and isUDP expected.");
...
static int Socket_isConnected(lua_State *L){
  if (!wlanInitialized) return luaL_error(L, wlanNotInitialized);
  if (lua_gettop(L) != 1) return luaL_error(L, "no argument expected.");
  
  Socket* socket = *toSocket(L, 1);
  if (socket->serverSocket || socket->udp) {
    lua_pushboolean(L, 1);
    return 1;
  }
...
static int Socket_createServerSocket(lua_State *L){
	if (!wlanInitialized) return luaL_error(L, wlanNotInitialized);
	if (lua_gettop(L) != 1) return luaL_error(L, "port expected.");
...
static int Socket_accept(lua_State *L){
	...
	if (socket->udp) return luaL_error(L, "accept only allowed for server sockets using the TCP protocol only.");
...
static int Socket_recv(lua_State *L){
  ...
  if (!socket->udp){ //TCP
    ...
    u32 cb;
    cb = sceNetInetRecvfrom(socket->sock, (u8*) &data, sizeof(data), 0, &addrFrom, &cbAddrFrom);
    if (cb == -1) return luaL_error(L, "something catastrophic happened, socket receive is locked up");
    if (cb > 0) {
      lua_pushlstring(L, data, cb);
      lua_pushnumber(L,cb);
    } else {
      lua_pushstring(L, "");
    }
...
static int Socket_send(lua_State *L){
  if (!wlanInitialized) return luaL_error(L, wlanNotInitialized);
  int result;
  if (lua_gettop(L) != 2) return luaL_error(L, "one argument expected.");
  ...
  if (!socket->udp){
    result = sceNetInetSend(socket->sock, string, size, 0);
  }else{
    int cbAddrTo = sizeof(socket->addrTo);
    result = sceNetInetSendto(socket->sock, string, size, 0,&socket->addrTo, cbAddrTo);
  }
  lua_pushnumber(L, result);
  return 1;
}
...
static const luaL_reg Wlan_functions[] = {
	...
	{"getConnectionState", Wlan_getState},
	{"getIPAddress", Wlan_getIPAddress},
	{"getEtherAddr", Wlan_GetEtherAddr},
	{"getSwitchState", Wlan_GetSwitchState},
	{0, 0}
};
