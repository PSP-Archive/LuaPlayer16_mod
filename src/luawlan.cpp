...
static int Wlan_useConnectionConfig(lua_State* L){
  ...
  if (argc != 1 && argc != 2) return luaL_error(L, "Argument error: index to connection config expected and an optional boolean for manual state checking.");
...
#define MAX_UDP_SAFE_BUFFER 256
static int Socket_recv_udp(lua_State *L)
{
	...
	//recieve
	        ...
	        char buffer[MAX_UDP_SAFE_BUFFER];
            struct sockaddr_in addrFrom;
            int cbAddrFrom = sizeof(addrFrom);
            u32 cb;
            cb = sceNetInetRecvfrom(socket->sock, (u8*) &buffer, sizeof(buffer), 0,
                            &addrFrom, &cbAddrFrom);
//                            
...
static int Socket_send_udp(lua_State *L)
{
	...
	int result = sceNetInetSendto(socket->sock, string, size, 0,
                            &socket->addrTo, cbAddrTo);
